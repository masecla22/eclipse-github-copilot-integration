...
@ThreadSafe
public final class BasicEditorRequest implements LanguageEditorRequest, Disposable {
	...
	@TestOnly
	public static BasicEditorRequest createGhostText(PsiFile file, LanguageSupport languageSupport, String text,
			int offset, LineInfo lineInfo) {
		if (file == null) {
			throw new IllegalStateException("file cannot be null!");
		}
		if (languageSupport == null) {
		...
		return new BasicEditorRequest(file.getProject(), languageSupport, CompletionType.GhostText, file.getLanguage(),
				file.getName(), text, offset, lineInfo, true, 4, 0);
	}

	public BasicEditorRequest(Project project, LanguageSupport language, CompletionType completionType,
			Language fileLanguage, String relativeFilePath, String documentContent, int offset, LineInfo lineInfo,
			boolean useTabIndents, int tabWidth, Integer documentModificationSequence) {
		if (project == null) {
	...
	public PsiFile createFile() {
		...
		return PsiFileFactory.getInstance((Project) this.project).createFileFromText(name, this.fileLanguage,
				(CharSequence) this.documentContent, false, false, true);
	}
	...
	public String toString() {
		return "BasicEditorRequest(language=" + this.getLanguage() + ", completionType=" + this.getCompletionType()
				+ ", useTabIndents=" + this.isUseTabIndents() + ", tabWidth=" + this.getTabWidth() + ", requestId="
				+ this.getRequestId() + ", fileLanguage=" + this.getFileLanguage() + ", relativeFilePath="
				+ this.getRelativeFilePath() + ", documentContent=" + this.getDocumentContent() + ", offset="
				+ this.getOffset() + ", lineInfo=" + this.getLineInfo() + ", requestTimestamp="
				+ this.getRequestTimestamp() + ", documentModificationSequence="
				+ this.getDocumentModificationSequence() + ", isCancelled=" + this.isCancelled() + ")";
	}

	public BasicEditorRequest(Project project, LanguageSupport language, CompletionType completionType,
			boolean useTabIndents, int tabWidth, int requestId, Language fileLanguage, String relativeFilePath,
			String documentContent, int offset, LineInfo lineInfo, Integer documentModificationSequence,
			boolean isCancelled) {
		if (relativeFilePath == null) {
			throw new IllegalStateException("relativeFilePath cannot be null!");
		}
	...
	public BasicEditorRequest withUseTabIndents(boolean useTabIndents) {
		return this.useTabIndents == useTabIndents ? this
				: new BasicEditorRequest(this.project, this.language, this.completionType, useTabIndents, this.tabWidth,
						this.requestId, this.fileLanguage, this.relativeFilePath, this.documentContent, this.offset,
						this.lineInfo, this.documentModificationSequence, this.isCancelled);
	}
