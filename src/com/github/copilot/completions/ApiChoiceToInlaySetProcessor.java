...
class ApiChoiceToInlaySetProcessor implements Flow.Subscriber<APIChoice> {
	...
	ApiChoiceToInlaySetProcessor(EditorRequest request, PromptInfo prompt, TelemetryData baseTelemetryData,
			Flow.Subscriber<List<CopilotInlayList>> subscriber, Consumer<APIChoice> onNewItem) {
		if (request == null) {
	...
	public void onError(Throwable throwable) {
		...
		TelemetryData data = TelemetryData.extend(this.baseTelemetryData, Map.of("cancelledNetworkRequest", "true"),
				String2DoubleMap.of("timeSinceIssuedMs",
						System.currentTimeMillis() - this.request.getRequestTimestamp()));
