...
public class CopilotPluginUpdater {
	...
	private static void notifyUpdateAvailable(Project project) {
		...
		Notification notification = CopilotNotifications.createFullContentNotification(
				CopilotBundle.get("pluginUpdate.updateNotification.title"),
				CopilotBundle.get("pluginUpdate.updateNotification.message"), NotificationType.IDE_UPDATE, true);
		notification.addAction((AnAction) NotificationAction.createSimpleExpiring(
				(String) CopilotBundle.get("pluginUpdate.updateNotification.installButton"),
				() -> ApplicationManager.getApplication()
						.executeOnPooledThread(() -> CopilotPluginUpdater.installCopilotUpdate(project))));
		notification.addAction((AnAction) NotificationAction
				.createSimpleExpiring((String) CopilotBundle.get("pluginUpdate.restartNotification.hideButton"), () -> {
	...
	private static void notifyUpdateUnavailable(Project project) {
		...
		Notification notification = CopilotNotifications.createFullContentNotification(
				CopilotBundle.get("pluginUpdate.noUpdateNotification.title"),
				CopilotBundle.get("pluginUpdate.noUpdateNotification.message"), NotificationType.IDE_UPDATE, true);
		notification.addAction((AnAction) NotificationAction.createSimpleExpiring(
				(String) CopilotBundle.get("pluginUpdate.noNotification.hideButton"),
				(Runnable) EmptyRunnable.INSTANCE));
		notification.notify(project);
	}
	...
	private static void installCopilotUpdate(final Project project) {
		...
		new Task.Backgroundable(project, CopilotBundle.get("pluginUpdate.updatingTask.title"), true,
			...
			public void run(ProgressIndicator indicator) {
				...
				PluginDownloader copilotUpdate = updateDownloaders.getAllEnabled().stream()
						.filter(p -> CopilotPlugin.COPILOT_ID.equals((Object) p.getId())).findFirst().orElse(null);
				...
				ProgressManager.getInstance().executeNonCancelableSection(() -> {
					try {
						...
						ApplicationManager.getApplication()
								.invokeLater(() -> CopilotPluginUpdater.notifyRequiredRestart(project));
					} catch (IOException e) {
	...
	private static void notifyRequiredRestart(Project project) {
		...
		boolean ok = MessageDialogBuilder.yesNo((String) CopilotBundle.get("pluginUpdate.restartNotification.title"),
				(String) CopilotBundle.get("pluginUpdate.restartNotification.content")).ask(project);
		if (ok) {
	...
	public static final class CheckUpdatesTask extends Task.Backgroundable {
		...
		public void run(ProgressIndicator indicator) {
			...
			try {
				InternalPluginResults availableUpdates = UpdateChecker.getInternalPluginUpdates();
				boolean updateAvailable = availableUpdates.getPluginUpdates().getAllEnabled().stream()
						.anyMatch(p -> CopilotPlugin.COPILOT_ID.equals((Object) p.getId()));
				if (updateAvailable) {
					StartupManager.getInstance((Project) this.project).runWhenProjectIsInitialized(
							() -> CopilotPluginUpdater.notifyUpdateAvailable(this.project));
				} else if (this.notifyNoUpdate) {
					StartupManager.getInstance((Project) this.project).runWhenProjectIsInitialized(
							() -> CopilotPluginUpdater.notifyUpdateUnavailable(this.project));
				}
			} catch (Exception e) {
