...
implements StartupActivity {
    ...
    public void runActivity(Project project) {
        ...
        Notification notification = CopilotNotifications.createFullContentNotification(CopilotBundle.get("earlyBirdNotification.title"), CopilotBundle.get("earlyBirdNotification.message"), NotificationType.WARNING, true);
        notification.addAction((AnAction)NotificationAction.createSimpleExpiring((String)CopilotBundle.get("earlyBirdNotification.removeChannelButton"), () -> {
            CopilotLocalApplicationSettings.settings().checkEarlyBirdChannel = false;
            boolean ok = UpdateSettings.getInstance().getStoredPluginHosts().removeIf(EARLY_BIRD_CHANNEL::equals);
            if (!ok) {
                LOG.warn("Failed to remove the early bird channel: " + hostListCopy);
            }
        }));
        notification.addAction((AnAction)NotificationAction.createSimpleExpiring((String)CopilotBundle.get("earlyBirdNotification.hideButton"), () -> {
            CopilotLocalApplicationSettings.settings().checkEarlyBirdChannel = false;
        }));
