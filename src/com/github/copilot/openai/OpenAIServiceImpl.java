...
public class OpenAIServiceImpl implements OpenAIService {
	...
	private final String[] staticHttpHeaders = new String[] { "Content-Type", "application/json", "User-Agent",
			"IntelliJCopilot/" + CopilotPlugin.getVersion(), "Openai-Organization", "github-copilot", "Editor-Version",
			CopilotPlugin.editorVersionString(), "Editor-Plugin-Version", CopilotPlugin.pluginVersionString() };
	...
	public void fetchCompletions(String apiToken, LanguageEditorRequest request, String prompt, int completionCount,
		...
		TelemetryData telemetryData = TelemetryData.create(
				Map.of("endpoint", "https://copilot-proxy.githubusercontent.com", "engineName",
						lang.getCopilotLanguage().getEngineName(), "uiKind",
						request.getCompletionType().getTelemetryPropertyValue()),
				String2DoubleMap.of("promptCharLen", prompt.length()));
		TelemetryService.getInstance().track("request.sent", telemetryData);
		String body = this.createCompletionBody(prompt, completionCount, temperature, maxTokens, topP,
				multilineCompletions ? lang.getMultiLineStops() : lang.getSingleLineStops(), blockMode,
				Math.max(0, request.getLineInfo().getNextLineIndent()));
		if (LOG.isTraceEnabled()) {
			LOG.trace("Sending completions request: " + body);
		}
		JsonToApiChoiceProcessor processor = new JsonToApiChoiceProcessor(request, telemetryBaseData,
				(Flow.Subscriber<APIChoice>) subscriber2);
		...
		response.whenComplete((httpResponse, ex) -> {
			...
			if (httpResponse != null) {
				...
				TelemetryData responseTelemetry = TelemetryData.extend(telemetryData,
						Map.of("headerRequestId", requestId, "serverExperiments",
								OpenAIHttpUtil.getServerExperiments(httpResponse.headers())),
						String2DoubleMap.of("totalTimeMs", totalRequestTime));
				TelemetryService.getInstance().track("request.response", responseTelemetry);
				TelemetryService.getInstance().trackSecure("engine.prompt",
						Map.of("prompt", prompt, "headerRequestId", requestId));
			} else if (OpenAIHttpUtil.isErrorException(ex)) {
				TelemetryService.getInstance().track("request.error", TelemetryData.extend(telemetryData,
						Map.of("message", ex.getMessage()), String2DoubleMap.of("totalTimeMs", totalRequestTime)));
			}
		});
	}

	protected String getCompletionURL(LanguageSupport lang) {
		String string = "https://copilot-proxy.githubusercontent.com" + lang.getCopilotLanguage().getEnginePath()
				+ "/completions";
		if (string == null) {
	...
	private CompletableFuture<HttpResponse<Object>> postStreaming(String url, String secretKey, String requestBody,
			EditorRequest editorRequest, JsonToApiChoiceProcessor jsonProcessor) {
		if (url == null) {
		...
		HttpRequest.Builder requestBuilder = HttpRequest.newBuilder(URI.create(url)).timeout(Duration.ofMillis(20000L))
				.header("Authorization", "Bearer " + secretKey).headers(this.staticHttpHeaders);
		...
		HttpRequest request = requestBuilder.POST(
				HttpRequest.BodyPublishers.ofString(requestBody == null ? "" : requestBody, StandardCharsets.UTF_8))
				.build();
		...
		response.whenComplete((resp, ex) -> {
			...
			if (LOG.isTraceEnabled()) {
				LOG.trace(String.format("%d: HTTP completion request: %d ms, ex: %s, resp: %d",
						editorRequest.getRequestId(), System.currentTimeMillis() - start, ex,
	 ...
	 * WARNING - void declaration
	 */
	private String createCompletionBody(String prompt, int count, double temperature, int maxTokens, int topP,
			String[] stopMarkers, BlockMode blockMode, int n) {
		void nextLineIndent;
		if (prompt == null) {
			throw new IllegalStateException("prompt cannot be null!");
		}
