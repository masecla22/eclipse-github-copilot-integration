...
public class CopilotStatusService implements CopilotStatusListener, Disposable {
	private final Object lock = new Object();
	@GuardedBy(value = "lock")
	private CopilotStatus status = CopilotStatus.Ready;

	public static CopilotStatus getCurrentStatus() {
		CopilotStatus copilotStatus = ((CopilotStatusService) ApplicationManager.getApplication()
				.getService(CopilotStatusService.class)).status;
	...
	public CopilotStatusService() {
		ApplicationManager.getApplication().getMessageBus().connect((Disposable) this)
				.subscribe(CopilotStatusListener.TOPIC, (Object) this);
	}
	...
	public static void notifyApplication(CopilotStatus status) {
		...
		((CopilotStatusListener) ApplicationManager.getApplication().getMessageBus()
				.syncPublisher(CopilotStatusListener.TOPIC)).onCopilotStatus(status);
	}
