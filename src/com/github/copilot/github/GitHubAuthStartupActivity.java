...
public class GitHubAuthStartupActivity implements StartupActivity.Background {
	...
	public void runActivity(Project project) {
		...
		GitHubCopilotToken copilotToken = service.fetchCopilotTokenInteractive(project, null, true,
				this::notifyUnauthorized, this::notifyTokenExpired);
		if (copilotToken != null && copilotToken.isValid()
				&& !CopilotApplicationSettings.settings().isTelemetryTermsAccepted()) {
	...
	private void notifyUnauthorized(Project project, String url) {
		...
		Notification notification = CopilotNotifications.createFullContentNotification(
				CopilotBundle.get("github.login.copilotUnauthorized.title"),
				CopilotBundle.get("github.login.copilotUnauthorized.message", url), NotificationType.WARNING, true);
		notification.addAction((AnAction) NotificationAction.createSimpleExpiring(
				(String) CopilotBundle.get("github.login.copilotUnauthorized.joinButton"),
	...
	private void notifyTokenExpired(Project project) {
		...
		Notification notification = CopilotNotifications.createFullContentNotification(
				CopilotBundle.get("github.login.copilotToken.unknownError.title"),
				CopilotBundle.get("github.login.copilotToken.unknownError.message"), NotificationType.ERROR, true);
		GitHubAuthStartupActivity.afterInit(project, () -> notification.notify(project));
	}
	...
	private void promptTelemetryTerms(Project project) {
		...
		Notification notification = CopilotNotifications.createFullContentNotification(
				CopilotBundle.get("github.login.telemetryConsent.title"),
				CopilotBundle.get("github.login.telemetryConsent.message"), NotificationType.WARNING, false);
		notification.addAction((AnAction) NotificationAction
				.createSimpleExpiring((String) CopilotBundle.get("github.login.telemetryConsent.yes"), () -> {
		...
		notification.addAction((AnAction) NotificationAction.createSimpleExpiring(
				(String) CopilotBundle.get("github.login.telemetryConsent.no"),
				() -> TelemetryService.getInstance().track("auth.telemetry_terms_rejected")));
		GitHubAuthStartupActivity.afterInit(project, () -> notification.notify(project));
	}
