...
public class CopilotStatusBarWidget extends EditorBasedStatusBarPopup {
	...
	public static void update(Project project) {
		...
		if ((widget = WindowManager.getInstance().getStatusBar(project)
				.getWidget(WIDGET_ID)) instanceof CopilotStatusBarWidget) {
			((CopilotStatusBarWidget) widget)
					.update(() -> WindowManager.getInstance().getStatusBar(project).updateWidget(WIDGET_ID));
		}
	}
	...
	protected EditorBasedStatusBarPopup.WidgetState getWidgetState(VirtualFile file) {
		...
		String toolTip = CopilotBundle
				.get(enabled != false ? "statusBar.tooltipForEnabled" : "statusBar.tooltipForDisabled");
		EditorBasedStatusBarPopup.WidgetState state = new EditorBasedStatusBarPopup.WidgetState(toolTip, "", true);
	...
	protected ListPopup createPopup(DataContext context) {
		String groupId = CopilotStatusService.getCurrentStatus() == CopilotStatus.Ready ? "copilot.statusBarPopup"
				: "copilot.statusBarErrorPopup";
		AnAction group = ActionManager.getInstance().getAction(groupId);
		if (!(group instanceof ActionGroup)) {
			return null;
		}
		return JBPopupFactory.getInstance().createActionGroupPopup(CopilotBundle.get("statusBar.displayName"),
				(ActionGroup) group, context, JBPopupFactory.ActionSelectionAid.SPEEDSEARCH, false);
	}
